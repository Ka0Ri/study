...
def train_model(
    ...
    dset = CIFAR10(dset_folder, download=True, train=True, transform=transforms.Compose(
                                [transforms.ToTensor(),
    ...
    dset_train_loader  = torch.utils.data.DataLoader(dset_train, batch_size=batch_size, shuffle=True, num_workers=2)
    dset_valid_loader = torch.utils.data.DataLoader(dset_valid, batch_size=batch_size, shuffle=False, num_workers=2)
    ...
    model = CGAT(num_features=util.NUM_FEATURES, num_classes=util.NUM_CLASSES) # Convolutonal Graph Attention Network
    if use_cuda:
        model = model.cuda()
...
def test_model(
    ...
    best_model = CGAT(num_features=util.NUM_FEATURES, num_classes=util.NUM_CLASSES) # Convolutonal Graph Attention Network
    ...
    test_dset = CIFAR10(dset_folder, download=True, train=True, transform=transforms.Compose(
                                [transforms.ToTensor(),
                                transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))]))
